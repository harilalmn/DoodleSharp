...
// 1-20 Mbps, and drives the frame capture for you." },
        ...
        private void InitializeMemberDescriptions()
        {
            _memberDescriptions = new Dictionary<string, string>
            {
                ...
                { "ConsoleOutput.OutputChanged", "Raised when the console contents change, so a view can refresh. Fires immediately for Clear and on the throttle interval for writes, NOT once per Log — so do not use it to count messages. A re-run that did not recompile raises it at most once, when its output is swapped in, and not at all when that output is identical to what is already displayed. The sender is the ConsoleOutput instance and the args carry nothing; re-read GetEntries in the handler." },
