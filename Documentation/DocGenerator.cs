    ...
    public class DocGenerator
    {
        ...
        private void InitializeSummaries()
        {
            _summaries = new Dictionary<string, string>
            {
                ...
                { "DxfExporter", "Exports shapes to AutoCAD DXF (R12 ASCII). Construct one with new DxfExporter(), then Export(shapes, filePath) to write a file or ExportToString(shapes) to get the text. Shapes with a native DXF equivalent keep it — a VCircle becomes a CIRCLE entity, not sixty-four chords — and everything else (dimensions, hatches, splines, groups) is decomposed into polylines rather than being silently dropped. A multi-line VText is written as one TEXT entity per line, stacked 1.2 text heights apart along the label's own down direction (R12 TEXT has no multi-line form), and VText.Justify is not applied — every line starts at the same point. Coordinates are passed straight through: one drawing unit is one DXF unit, Y up." },
...
// override the box width used by GetBounds and anchoring. Note that GetBounds()
// estimates ONE line's box (0.6 x Height per character, a single Height tall), so
// set Width on a multi-line label if its bounds matter for picking or zooming.
...
// LIMITATION: justification is a CANVAS effect. SVG and PDF write Content as one
// run, so a multi-line label exports as a single line; DXF keeps the lines (one
// TEXT entity each) but starts them all at the same point -- R12 TEXT has no block
// width to justify inside." },
...
// 1-20 Mbps, and drives the frame capture for you." },
        ...
        private void InitializeMemberDescriptions()
        {
            _memberDescriptions = new Dictionary<string, string>
            {
                ...
                { "VText.Justify", "Gets or sets how the lines of a MULTI-LINE label line up with each other inside the text block (VTextJustify: Left, Center, Right). Default Left, and the lines come from newline characters (\\n) in Content. Composes with Anchor rather than competing with it: Anchor puts the block on the drawing, Justify decides the shape of the ragged edge inside it, so Anchor = MiddleCenter with Justify = Center is a block centred on its point whose short lines are also centred against its long ones. Single-line text is unaffected. It is pure layout inside the block: it never moves or resizes it, GetBounds() is identical for all three values, and Clone() carries it over. Export limitation: justification is a canvas-only effect. SVG and PDF write Content as one run, so a multi-line label lands as a single line and there is nothing to justify; DXF does keep the lines (one TEXT entity each, stacked 1.2 x Height apart) but starts them all at the same point, because R12 TEXT has no block width to justify inside." },
                ...
                { "VText.GetBounds", "Returns the axis-aligned bounding box of the text, honouring Anchor (which corner of the box is at Location) and Angle (the box is rotated and re-fitted). The box is an ESTIMATE, not a measurement: with Width left at 0 the width is taken as 0.6 x Height per character and the height is always a single Height, so a multi-line label reports one long, one-line-tall box rather than a stack of lines. Set Width when the bounds matter. Justify does not affect it." },
                ...
                { "VEllipse(VXYZ, double, double)", "A full ellipse about centre, radiusX along X and radiusY along Y. Equal radii give a circle. There is no rotation property — the axes are always world-aligned, which is also the assumption VEllipse.Contains rests on. Default stroke colour is Pink." },
                { "VEllipse(double, double, double, double)", "The same full ellipse with the centre written as two coordinates." },
                { "VEllipse(VXYZ, double, double, double, double)", "An elliptical ARC: the sweep runs counter-clockwise from startAngle to endAngle, both in DEGREES. A partial sweep encloses no area, so Contains becomes an on-the-curve test rather than an interior one. The curve is ARC-LENGTH parameterised, so Evaluate(t) and Divide(n) space points evenly along the curve rather than evenly in angle — EvaluateByAngle is the angle-linear reading." },
