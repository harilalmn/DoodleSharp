...
// 1-20 Mbps, and drives the frame capture for you." },
        ...
        private void InitializeMemberDescriptions()
        {
            _memberDescriptions = new Dictionary<string, string>
            {
                ...
                { "SvgTile.Shapes", "What is placed on this cell — IEnumerable<IDrawable>. Note the difference from PdfTile.Shapes, which is IReadOnlyList<IDrawable>. In the app this comes from CanvasRenderer.GetShapes(leaf)." },
                ...
                { "PdfTile.Shapes", "What is placed on this cell — IReadOnlyList<IDrawable>. Note the difference from SvgTile.Shapes, which is the looser IEnumerable<IDrawable>. In the app this comes from CanvasRenderer.GetShapes(leaf)." },
...
namespace StartViz
{
    public class Viz
    {
        public static void Main()
        {
            ...
            AddListItem(tipsList, "Code Runs on F5", "Nothing runs while you type. Press F5 or click Run when you want the canvas rebuilt - there is no auto-update setting, and no auto-draw setting either (shapes still appear on construction, which is Shape.AutoRegister and is on by default)");
