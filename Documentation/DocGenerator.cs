    ...
    public class DocGenerator
    {
        ...
        private void InitializeSummaries()
        {
            _summaries = new Dictionary<string, string>
            {
                ...
                { "VText", "Represents text drawn at a specific position. Supports font size via Height property or constructor parameter. Constructors: VText(point, text), VText(point, text, height), VText(x, y, text), VText(x, y, text, height). Supports Font, FontWeight, Anchor, and Angle properties for styling, alignment, and rotation. Mask draws a solid rectangle behind the glyphs so a label stays readable where it crosses other geometry, with MaskColor (default \"Black\") and MaskOffset (padding as a fraction of the text height, 0 to 1, default 0.15). Individual characters can be converted to vector outline shapes: ToCharShape(i) (non-mutating), LiftChar(i) and the indexer text[i] (extract the glyph as a shape AND replace the character with a space), and LiftChars(start, count) for a selection. These let you morph a letter into another shape, e.g. new TransformAnimation(text[0], circle, 2)." },
...
dim.MaskColor = ""Black"";        // any colour name or hex — VColor.Black, ""#202020"", …
