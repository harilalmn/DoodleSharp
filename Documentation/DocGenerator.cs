    ...
    public class DocGenerator
    {
        ...
        private void InitializeSummaries()
        {
            _summaries = new Dictionary<string, string>
            {
                ...
                { "VPoint", "Represents a visible point marker on the canvas — a drawn dot, not a coordinate. For coordinates and vectors use VXYZ; constructing a VPoint auto-registers a shape. Constructors: (x, y) and (VXYZ position). X and Y are settable. Converts to VXYZ implicitly, or explicitly via AsVXYZ(). Operators (not listed in the member tables below): + and - between a VPoint and a VPoint or VXYZ, in either order; * and / by a double (p * s, s * p, p / s); and * and / between two points (VPoint with VPoint or VXYZ), which are COMPONENT-WISE — (2, 3) * (4, 5) is (8, 15). Every overload returns a plain VXYZ so intermediates never pollute the canvas. A VPoint counts as (X, Y, 0): + and - carry a VXYZ operand's Z through (a - subtracts it), while * and / always give Z = 0. There is no unary minus and no VPoint + double. EQUALITY TRAP: a VPoint is a shape, so == between two VPoints compares IDENTITY — two markers at the same position are not ==. Compare positions as VXYZ, a.AsVXYZ() == b.AsVXYZ(), which is fuzzy to 1e-9; a VPoint == VXYZ comparison converts on its own and is fuzzy too. Default Color and FillColor are both White, and unusually they are assigned OUTRIGHT rather than through ShapeDefaults: VPoint is the one shape that does NOT honour ShapeDefaults.GlobalColor / GlobalFillColor, so set point.Color yourself if you are styling globally. ON-SCREEN SIZE: a point has no size of its own — its bounds are the single coordinate — so the marker is a fixed number of DEVICE PIXELS and does not grow or shrink as you zoom. It is a 1.5-pixel-radius dot by default, or a 5-pixel-radius disc with Settings → \"Draw Point As Patch\" on; LineWeight does not govern it, and it is never dropped by the level-of-detail pass however far out you zoom. If you want a mark that scales with the drawing, use a small VCircle instead. Because the bounds are a bare point, a scene of several VPoints still zooms to fit them; a scene of exactly ONE has a zero-sized extent, so Zoom to Fit centres on it at a default 100-unit view rather than zooming in without limit." },
                ...
                { "RayHit", "Readonly record struct returned by RayCaster.FindIntersection (as RayHit?, where null means the ray hit nothing) and, one per query, by FindIntersections. Positional properties: Shape (the hit shape), Point (VXYZ world-space hit location), Distance (Euclidean distance from the ray origin to the hit point, in world units). Construct it with new RayHit(shape, point, distance), though you rarely need to. Being a record struct it has value equality — == and != compare all three members: Shape by reference (shapes do not define value equality), Point with VXYZ's fuzzy equality (1e-9) and Distance exactly — plus Deconstruct, so var (shape, point, distance) = hit works, and a readable ToString()." },
                { "RayQuery", "Readonly record struct used by RayCaster.FindIntersections to describe a single ray. Positional properties: Origin (VXYZ, where the ray starts) and Direction (VXYZ; need not be normalised, and Z is ignored — queries run on the XY plane). Construct it with new RayQuery(origin, direction). Being a record struct it has value equality — == and != compare both vectors with VXYZ's fuzzy equality (1e-9), so two queries built from the same numbers are equal — plus Deconstruct and a readable ToString()." },
                ...
                // Support classes
                { "VXYZ", "3D coordinate type (X, Y, Z) used for every position, vector and direction parameter in the library — the counterpart to Revit's XYZ. Its components are read-only: every operation returns a new instance, so a VXYZ can be shared without aliasing bugs. Constructors: (x, y, z), (x, y) with Z = 0, () for the origin, and the polar (angleInDegrees, distance, fromPoint = null) — the point 'distance' away from fromPoint (VXYZ.Zero when omitted) at an angle in degrees counter-clockwise from +X, with Z taken from fromPoint. Beware: new VXYZ(45, 100) with two plain numbers is ALWAYS the Cartesian (x, y), never polar; write new VXYZ(45, 100, VXYZ.Zero) or new VXYZ(angleInDegrees: 45, distance: 100). Never registers on the canvas — use it freely for intermediate maths, and reach for VPoint only when you want a dot drawn. Vector operations: Add, Subtract, Multiply, Divide, Negate, Normalize (returns Zero for a zero-length vector rather than throwing), GetLength, DistanceTo, DotProduct, CrossProduct, TripleProduct, AngleToDegrees (unsigned, 0 to 180 - the library's convention) and AngleToRadians (0 to π), Rotate(degrees) about the Z axis, Clone, AsVPoint. The unit-less AngleTo is OBSOLETE: it returns radians, which is the one place this library does not work in degrees, and handing its answer to a degrees-taking API is silently wrong rather than obviously wrong. Tests: IsZeroLength, IsUnitLength, IsAlmostEqualTo(other, tolerance = 1e-9), static IsWithinLengthLimits. Indexer [0]/[1]/[2] reads X/Y/Z and throws IndexOutOfRangeException otherwise. Operators (the member tables below do not list operators, so here they are in full): a + b and a - b between two VXYZ; v * s, s * v and v / s with a double (dividing by 0 gives infinite or NaN components, not an exception); and unary -v, the same as Negate(). There is NO VXYZ * VXYZ — use DotProduct or CrossProduct. Mixed arithmetic with a VPoint is declared on VPoint and always returns a plain VXYZ, never a drawable point. == and != are fuzzy: every component within an absolute 1e-9 (IsAlmostEqualTo's default); null == null is true and a value never equals null. GetHashCode rounds to 8 decimals, which groups nearly-equal values but cannot agree with the tolerance everywhere — two values either side of a rounding boundary are == yet hash differently — so compare computed points with == or IsAlmostEqualTo rather than relying on a HashSet or Dictionary to merge them. Static properties: Zero, BasisX, BasisY, BasisZ." },
                ...
                // Hatch Patterns
                { "VHatch", "Fills a closed polygon boundary with a repeating line pattern. Supports 72 built-in AutoCAD-standard patterns (via BuiltInHatch enum or name string) and custom patterns defined using the .pat format. VHatch is auto-named like every other shape: `var h = new VHatch(...)` takes the name h and stays visible after Main() returns. Only a hatch without a declared variable of its own — added straight to a list, or a method result held in var — needs a Name or Place(). Constructors: new VHatch(polygon, BuiltInHatch.ANSI31, scale, angle), new VHatch(polygon, \"BRICK\", scale, angle), new VHatch(polygon, hatchType, scale, angle), new VHatch(boundaryPoints, pattern, scale, angle). Static factory: VHatch.FromDefinition(polygon, patString, scale, angle). Properties: Boundary (List<VXYZ>), Pattern (HatchType), PatternScale (double), PatternAngle (double), Color, LineWeight, Opacity. Methods: GenerateLines() returns clipped line segments, Clone(), Move(), Rotate(), Flip(), Scale(), GetBounds(), Contains(point) (an exact test against the boundary, not the bounding box), DistanceTo(point) (to the boundary treated as a closed path)." },
                ...
                { "ParamValue", "The self-converting value returned by GlobalParameters.Get(name). Converts implicitly to double, bool, string and DateTime so a parameter reads naturally without a type argument. Named accessors Num, Flag, Text, Date plus Raw, Exists, Name and As<T>() are always unambiguous. Caveat: because it converts to both double and string, the + operator cannot pick an overload — Get(\"n\") + 1 is a compile error; use Get(\"n\").Num + 1 or GlobalParameters.Get<double>(\"n\") + 1. The conversions are operators, which the member tables below do not list: IMPLICIT to double, bool, string and DateTime; EXPLICIT (a cast) to int and float, for the same ambiguity reason — an implicit int would make Get(\"n\") * 2 ambiguous. (int) ROUNDS with Math.Round, so halves go to the even neighbour (2.5 → 2, 3.5 → 4) instead of truncating the way a C# cast of a double does; (float) is a plain narrowing. Every conversion throws InvalidOperationException when the parameter is undeclared or holds a different kind (a bool parameter read as a double, say), naming the parameter; ToString() never throws and returns \"\" for an undeclared one." },
...
VXYZ half = marker / 2;                         // * and / by a number too; Z is always 0

// == between two VPoints is IDENTITY (they are shapes), not position:
var a = new VPoint(10, 10);
var b = new VPoint(10, 10);
bool sameShape = a == b;                        // false
bool samePlace = a.AsVXYZ() == b.AsVXYZ();      // true - VXYZ == is fuzzy (1e-9)
...
// The marker is a FIXED number of screen pixels (1.5 px radius, or 5 px with
// Settings -> Draw Point As Patch): it does not grow when you zoom in, and
// LineWeight does not size it. For a mark that scales with the drawing, use
// a small circle instead:
var scalingMark = new VCircle(new VXYZ(60, 0), 3) { FillColor = ""Yellow"" };" },
...
// Polar: a point 'distance' away from 'fromPoint', at an angle in DEGREES
// counter-clockwise from +X (90 is straight up - the canvas is Y-up).
var hub = new VXYZ(20, 10);
var spoke = new VXYZ(90, 50, hub);              // (20, 60, 0): 50 above hub
var back  = new VXYZ(90, -50, hub);             // (20, -40, 0): negative flips the side
var marker = new VPoint(0, 0);                  // draws a dot
var tip   = new VXYZ(30, 80, marker);           // a VPoint works as fromPoint

// TRAP: two plain numbers are ALWAYS Cartesian, never polar.
var cartesian = new VXYZ(45, 100);              // the point (45, 100)
// Polar from the origin needs the third argument, or named arguments:
var polar1 = new VXYZ(45, 100, VXYZ.Zero);      // (70.71, 70.71, 0)
var polar2 = new VXYZ(angleInDegrees: 45, distance: 100);   // same point

// Points evenly round a circle of radius 100 at the origin:
for (int i = 0; i < 6; i++)
    new VPoint(new VXYZ(i * 60, 100, VXYZ.Zero));
...
// Operators between VXYZ values: + and -, * and / by a number, unary minus.
var a = new VXYZ(1, 2);
var b = new VXYZ(3, 4);
var sum   = a + b;            // (4, 6, 0)
var diff  = b - a;            // (2, 2, 0)
var twice = 2 * a;            // (2, 4, 0) - a * 2 works too
var half  = b / 2;            // (1.5, 2, 0)
var neg   = -a;               // (-1, -2, 0), the same as a.Negate()
// There is no VXYZ * VXYZ: use DotProduct or CrossProduct.

// == and != are FUZZY: every component within 1e-9.
bool equal = new VXYZ(0.1 + 0.2, 0) == new VXYZ(0.3, 0);   // true
bool apart = new VXYZ(1, 0) == new VXYZ(1.001, 0);         // false

// Mixing in a VPoint (a drawn marker) also returns a plain VXYZ.
var fromMarker = a + marker;  // (1, 2, 0): marker is the VPoint at the origin above" },
...
double dx = 30, dy = 30;
double heading = Math.Atan2(dy, dx).ToDegrees();   // 45
...
// A method result is named after its variable only when the declaration spells
// out the shape type; held in var (or in a list) it is unnamed, and Place() keeps it:
...
VPolygon? merged = a.Union(b);   // named ""merged"" (explicit type), but default styling
...
// in the console, tagged ""Geometry"" — via GeometryDiagnostics. Declared with its
// type, as here, the result is named after the variable and kept; held in var, or
// as a piece of a list, it is unnamed — name it (or call Place()) or the post-run
// sweep hides it.
...
var holedDiff = BooleanOps.DifferenceWithHoles(poly1, poly2);
...
var simplePieces = BooleanOps.MakeSimple(poly1);
...
variant.Lines[0].Dashes = new[] { 4.0, -2.0 };" },
                { "BuiltInHatch", @"// Use enum values for built-in patterns. Each var is auto-named after its
// variable (h1, h2, ...), so the explicit Names here are optional.
...
// ticks, labels and data shapes. Held in var it carries no Name, so give it
// one — or declare it VGroup revenue = Chart.Bar(...), which names it after
// the variable — or the unnamed-shape sweep will hide it.
...
int    ri = (int)GlobalParameters.Get(""Radius"");     // explicit -> int, ROUNDED half-to-even
                                                     // (2.5 -> 2, 3.5 -> 4), never truncated
float  rf = (float)GlobalParameters.Get(""Radius"");   // explicit -> float
double d  = GlobalParameters.Get(""Radius"").Num;      // named accessor

// + is the one ambiguous operator (double + double vs string + object):
// double bad = GlobalParameters.Get(""Radius"") + 1;  // does NOT compile
double ok = GlobalParameters.Get(""Radius"").Num + 1;  // Num (or Get<double>) fixes it
...
mau.Name = ""mau"";           // held in var, a chart is unnamed — name it (or declare it VGroup)
...
// 1-20 Mbps, and drives the frame capture for you." },
        ...
        private void InitializeMemberDescriptions()
        {
            _memberDescriptions = new Dictionary<string, string>
            {
                ...
                { "Shape.Name", "Optional label for the shape, default an empty string. Also load-bearing for visibility: after Main() returns, shapes whose Name is empty and which were never explicitly drawn are hidden as construction leftovers. Before the code runs, an auto-naming pass fills it with the variable's name for var x = new VShape(...) and for any local or field declaration that spells out the shape type (VPolygon? merged = a.Union(b)), but never overwrites a Name that is already set. var x = SomeMethod() and shapes held in lists are not named." },
                ...
                { "VXYZ.Z", "Gets the Z component. Read-only. 0 from the two-argument (x, y) constructor and from (); copied from fromPoint by the polar constructor VXYZ(angleInDegrees, distance, fromPoint), which is 0 when fromPoint is omitted. Ignored by all 2D rendering." },
                ...
                { "VXYZ.AsVPoint", "Converts this VXYZ to a VPoint (ignores Z component). The VPoint is a drawable marker and registers on the canvas as it is constructed. Declare the result as VPoint (VPoint marker = v.AsVPoint();) so it is named after its variable and survives the post-run sweep; held in var it is unnamed and hidden after Main() returns. For a coordinate, keep the VXYZ." },
                ...
                { "VXYZ.GetHashCode", "Returns a hash code built from the components rounded to 8 decimal places, so that values considered equal by IsAlmostEqualTo usually hash together in dictionaries and sets. Usually, not always: rounding cuts the number line at fixed boundaries and the 1e-9 tolerance does not, so two values a hair either side of one (5e-9 + 1e-10 and 5e-9 - 1e-10, say) are == yet hash differently, and a HashSet<VXYZ> keeps both. Do not rely on hashing to merge computed points; compare with == or IsAlmostEqualTo." },
                ...
                { "Mouse.HasHandlers", "True while at least one handler is registered — which is also exactly what puts the canvas into interactive mode (click-to-select suppressed, double-click-zoom-to-fit suppressed, the F4 properties panel hidden and its menu item disabled). WHEEL ZOOM IS NOT PART OF THAT: the wheel is given up only when a wheel handler is registered, which HasWheelHandler reports separately, because losing the wheel would cost a script that merely watches clicks the main way to navigate a drawing larger than the viewport. Middle-drag panning, the drawing tools and the measuring tape are unaffected too. It also decides whether Auto-Run keeps ticking: while this is true an Auto-Run tick whose source has not changed does nothing at all, because re-running Main() would clear the shapes your handlers drew; an edit, or the Run button, still runs the program. Read-only, and cheap: it is a plain field read, because the host tests it on every mouse event. Static." },
                ...
                { "VHatch.Clone", "Returns an independent copy: the boundary points are cloned, the pattern and its scale and angle are carried over, and the styling is copied. Like every Clone, the copy has no Name. Unlike most shapes' Clone(), this one is typed Shape rather than VHatch, so var copy = hatch.Clone() is a Shape and is not auto-named — mark it with Place(), set a Name, or cast it (VHatch copy = (VHatch)hatch.Clone(); is named copy), or the post-run sweep will hide it." },
                ...
                { "ConsoleOutput.Clear", "Removes every entry and refreshes the panel immediately (no throttling on this one). This is what the console's Clear button does. It does not affect what your code has already written elsewhere — there is only one buffer. The one exception to \"immediately\": called during a re-run that skipped compilation (an Auto-Run tick on unedited code, or a Global Parameters change), the panel catches up when the run returns a few milliseconds later rather than at once." },
                ...
                { "ConsoleOutput.GetEntries", "Returns a snapshot of every ConsoleEntry currently in the console, as an IReadOnlyList. It is a copy taken under the lock, so it is safe to enumerate while other threads keep writing. Use it to assert on your own output, or to save a run's log. It reports the console as the running program sees it — everything written so far in this run — so logging and then reading back works on every run path." },
                ...
                { "ConsoleOutput.OutputChanged", "Raised when the console contents change, so a view can refresh. Fires immediately for Clear and on the throttle interval for writes, NOT once per Log — so do not use it to count messages. A re-run that did not recompile raises it at most once, when its output is swapped in, and not at all when that output is identical to what is already displayed - and an Auto-Run tick that stands down (interactive code, or a source whose last run failed) runs nothing, so it raises nothing either. The sender is the ConsoleOutput instance and the args carry nothing; re-read GetEntries in the handler." },
                ...
                { "VXYZ(double, double, VXYZ)", "A polar point: the point 'distance' away from 'fromPoint' in the direction 'angleInDegrees', measured in DEGREES counter-clockwise from +X (so 90 is straight UP — the canvas is Y-up). Z is copied from fromPoint; the angle only turns in the XY plane. A null or omitted fromPoint means VXYZ.Zero, and a VPoint works as fromPoint through its implicit conversion to VXYZ. A negative distance lands on the opposite side (new VXYZ(0, -50, c) is 50 to the LEFT of c); a distance of 0 returns a copy of fromPoint. TRAP: new VXYZ(45, 100) is NOT polar — two plain numbers always bind to the Cartesian VXYZ(x, y), because C# prefers the overload that needs no default filled in, so you get the point (45, 100). For polar-from-origin, pass the third argument, new VXYZ(45, 100, VXYZ.Zero), or name the arguments, new VXYZ(angleInDegrees: 45, distance: 100), which the Cartesian constructor's x/y names cannot match." },
        ...
        public FlowDocument GenerateWelcomePage()
        {
            ...
            AddListItem(stepsList, "Keep Shapes Visible", "Shapes appear when constructed; call .Place() on method results held in var or in a list (VPolygon? u = a.Union(b) - a result declared with its shape type - is kept automatically)");
