...
- **A method result keeps its variable's name when the declaration spells out the shape type** — `VPoint vp1 = p1.AsVPoint();` was hidden after the run as an "unnamed shape", because only a `new` expression was named after its variable. Any shape-typed declaration now is; `var` with a method call still is not, since the rewriter cannot see which calls return shapes.
- **Points are drawn** — since the level-of-detail work in 2026.8.x no `VPoint` had been drawn at all: the "too small to see" test measured a point by its bounds, which are zero wide. A point is a marker of fixed screen size and now draws on every backend, at the same size on each, and the software rasterizer honours *Draw point as patch*.
- **Auto rendering stops flipping backends on small drawings** — the first raster frame after a switch was timed as though it were a vector frame, so after one slow frame Auto alternated renderers on every repaint.

### Version 2026.8.17 (Implemented) — Auto-Run Stands Down for an Interactive Sketch
- **Auto-Run leaves an interactive program alone** — once your code registers a `Mouse` handler, the shapes on the canvas are partly yours: a handler that draws builds a picture out of your input, and nothing but that input can rebuild it. Re-running would clear the canvas and draw only what `Main()` draws, so Auto-Run suspends the re-running until you edit the code — which still re-runs it at once. It says so in the console rather than going quiet, and your Auto-Run setting is untouched: the checkbox stays ticked and nothing is written to the project file, because the timer is also what notices your edits.
- **A failed run pauses Auto-Run instead of repeating itself** — a run whose `Main()` threw is not kept loaded, so every following tick was a full recompile, blanking the canvas while Roslyn worked and clearing and rewriting the console twice a second. A single null reference in user code therefore looked like a flickering console and a canvas that kept emptying. Auto-Run now pauses and says why; the next edit resumes it.
- **Runtime errors on a silent run are reported** — an Auto-Run or Global Parameters run that compiled and then threw produced no console line and left the status bar reading "Ready", because failure was reported by counting compile diagnostics and such a run has none. The error now reaches the console exactly as it does when you press Run.

### Version 2026.8.15 (Implemented) — Seven Editor Faults from Live Use
- **Tab works with multiple cursors** — it was never claimed by the multi-cursor handler, so the editor indented the first line and outdented the rest in the same keystroke and the cursors drifted apart. Tab now carries every cursor to its own next tab stop, and Shift+Tab strips one indent level from every line a cursor is on.
- **Copy and paste across multiple cursors keeps the fragments apart** — copying four words under four cursors and pasting put all four words in all four places. When the clipboard holds exactly one line per cursor, each cursor now takes its own line; anything else still pastes whole at every cursor.
- **Enter inside a string no longer breaks the code** — a line break between the quotes of `"…"` or `$"…"` is not legal C#, so pressing Enter there stopped the file compiling and the error that followed pointed at the quote rather than at the key. The literal is now closed and continued on the next line, caret ready inside the reopened quote. Verbatim and raw strings, which already accept line breaks, are untouched.
- **Duplicating a line leaves the caret on the copy** — with the caret at the end of the line, Alt+Shift+Down landed it a line too far down.
- **The completion list inside a function call offers what you can pass** — `Draw(` listed every type, method and keyword in scope and buried the two or three names you were reaching for. It now shows the values you already have — locals, parameters, fields and properties — filtered as you type, plus the type the parameter expects. Typing `new` brings the full list of types back. The trade is that a static call written as an argument is no longer offered from the list, though typing it still works.
- **`get` and `set` are offered inside a property** — typing `{ get` suggested `GetHashCode` and `GetType`, neither of which compiles there, and never the accessor itself.
- **A property initialiser knows its own type** — `public List<string> Names { get; set; } = new ` suggested nothing after `new`; it now opens on `List<string>`, as a local declaration already did. Parameter defaults gained the same.
