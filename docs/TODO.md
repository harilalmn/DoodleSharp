...
symptom names. Note 143.
- [x] **An Auto-Run tick no longer re-runs an interactive program** — a `Mouse.OnMove` handler that draws builds its picture out of the *user's* input, and a tick clears the canvas and re-runs `Main()`, which draws only what `Main()` draws. Nothing re-creates the rest, because nothing recorded it, so everything the user had drawn vanished twice a second. The guard is `MainWindow.ShouldStandDown()`, asking note 95's own `IsCanvasInteractive`; editing the code still re-runs it, because an edit changes the signature and never reaches the test. Deliberately **not** implemented by stopping the timer or unticking the checkbox: the tick is also the change detector, so a genuinely stopped Auto-Run would lose edit-then-re-run, and unticking would write `AutoRun: null` into the user's `.vizproj` behind their back.
  - Announced once per episode (`AnnounceInteractivePause`), because a re-run loop that silently stops is indistinguishable from one that is broken — which is how this was reported in the first place. Latched, never per tick: announcing per tick would refill the console at 2 Hz, which is the flicker being removed wearing a different hat.
- [x] **A failed run no longer retries itself at 2 Hz** — and this, not the exception, was the reported flicker. `ExecuteAssemblyAsync` captures the resident assembly only inside `if (result.Success)`, so a `Main()` that threw left `HasResidentAssembly` false and the tick's `|| !ModuleCompiler.HasResidentAssembly` sent **every** tick down the *full compile* path — which clears the canvas before Roslyn starts and clears the console before writing the banner. A single `NullReferenceException` in user code therefore presented as a flickering console and a canvas that kept emptying. `_autoRunSourceFailed` latches it; any source change clears it, as does `ApplyAutoRunSetting` on project open.
- [x] **A silent run that threw is reported instead of counted** — `RunSilentlyAsync` reported failure by counting error *diagnostics*, which is right for a compile failure and silent for a run that compiled and then threw, because those carry none. The count was zero, no branch fired, and the status bar read `Ready` while `Main()` died every 500 ms. `ReportSilentRunFailure` is now the one reporting point for both silent paths and writes `result.Error` to the console the way `RunButton_Click` always has.
  - Worth keeping: the fix was localised by reading, but *confirmed* by the journal. The reported project, copied out and flipped to `AutoRun: true`, ran twelve seconds — twenty-four ticks — and logged `EXEC.MAIN.INVOKE` once, `EXEC.RESIDENT.RERUN` zero times, `EXEC.MAIN.RETURN … shapes=6`. Six is the tell: four offset lines plus a `VPoint` and a `VText` for `p2`, the only one of four intersections that exists at the sketch's *starting* geometry — exactly the "everything vanished except p2" the user described.
  - `MainWindow.xaml.cs`, `Tests/AutoRunSettingTests.cs` (6 new), `Tests/AutoUpdateRemovalTests.cs` (two proximity windows widened — they guard "the run is inside the tick", not the tick's size). CLAUDE.md note 143.

### Recently Completed (2026-08-20) — The Flickering Line, and the Checkbox That Closed the App
- [x] **The console stopped flickering under Auto-Run** — reported from a screenshot, because the symptom was stranger than "it all flickers": one line blinked on its own while the rest looked steady. `ReExecuteResidentAsync` opened each 500 ms re-run with `ConsoleOutput.Instance.Clear()` and then wrote the identical output back, announcing a change at every step, so the panel emptied and refilled twice a second for a program nobody was editing. The line that stood out was the unnamed-shape warning, because `HideUnnamedShapes` writes it *after* `Main()` returns — inside the 50 ms update throttle — so it waited for the timer while the line above it came back at once. A run now stages its output (`ConsoleOutput.BeginRewrite`/`EndRewrite`, internal) and swaps it in only if the text differs, which for an unchanged program is never; `MainWindow.RefreshConsole` updates an `ObservableCollection` in place instead of reassigning `ItemsSource`, so scroll position and selection survive a refresh. `Console/ConsoleOutput.cs`, `Execution/ModuleCompiler.cs`, `MainWindow.xaml.cs`, `Tests/ConsoleRewriteTests.cs` (5). CLAUDE.md note 136.
  - The staging nearly introduced a quieter bug than the one it fixed: user code runs *inside* the rewrite, so `ConsoleOutput.GetEntries()` — documented for reading your own output back — would have returned the *previous* tick's lines on any run that skipped compilation. Caught in the documentation pass, by someone reading the code rather than the summary of it. `GetEntries`/`GetFormattedOutput` now answer for the running program and the internal `GetDisplayedEntries`/`GetDisplayedOutput` answer for the panel; the two differ only while a run is in flight.
  - Worth keeping: the canvas half was never at fault and needed no change. `CanvasRenderer.Clear()` only mutates its own list, and `Main()` runs on a worker thread, so `RenderCanvas` keeps drawing the previous scene until `RenderTo` replaces it in one step. Changing it would have been a fix to something that was not broken.
- [x] **Unticking Auto-Run could close the app** — and did, in the same session. `AutoRunCheck_Changed` called `SaveProjectFile()`, OneDrive had the `.vizproj` open for the instant `File.Replace` needed it (`ERROR_UNABLE_TO_REMOVE_REPLACED`, 0x80070497), and the `IOException` walked the WPF event route into the dispatcher. Note 134's rule, which only `async void` handlers were being held to. `DurableFile` now retries the final rename for about half a second — `IOException` only, so a read-only or ACL-denied destination still fails at once — and `MainWindow.TrySaveProjectFile` reports a failed write to the status bar instead of ending the process, on the reading that the project file records preferences already applied in memory. Saving the user's *source* keeps its loud failure. `Project/DurableFile.cs`, `MainWindow.xaml.cs`, `AddReferenceWindow.xaml.cs`, `Tests/ProjectSaveSafetyTests.cs`, `Tests/DurableFileTests.cs`. CLAUDE.md note 137.
  - Worth keeping: the crash journal named the culprit outright — `AutoRunCheck_Changed` → `SaveProjectFile` → `DurableFile.WriteAllText` → `ReplaceFile`, with `terminating=True` and the whole editor buffer beside it. Four first-chance records of the same `IOException` in seven milliseconds showed it was a lock, not a permission.

### Recently Completed (2026-08-20) — The Blank Canvas, a Usable Completion Filter, and Multi-Line Justification
- [x] **The canvas drew nothing while the status bar said it had** — a stock, undivided layout, every run. `CanvasRenderer.Clear()` calls `Viewport.Reset()`, which does not empty the root but **installs a brand-new root object**; the cells keep the previous one in `OwningViewport` until `ViewportHost.Sync()` re-keys them, and `Sync()` is queued at `DispatcherPriority.Render` — below the Normal-priority await continuation that runs the render. So `RenderTo` reliably asked `GetShapes(leaf)` for a viewport that had just left the tree, `FirstLeaf()` returned the dead node, and every cell was handed `Array.Empty`. The count in the status bar came from `GetShapes()` with no viewport, which is why it reported `Success: 3 shapes drawn` over an empty canvas, and why ⤢ zoom-to-fit took the empty-list branch and reset to 100% rather than framing anything. Fixed by resolving the argument with `ResolveVisible()` — the same rule the shapes in that method already follow, and identical to `FirstLeaf()` for any attached viewport. `Canvas/CanvasRenderer.cs`, `Tests/ViewportPlacementTests.cs`. CLAUDE.md note 123.
  - Worth keeping: the renderer was never at fault. The note 69 offscreen harness drew the same three shapes correctly throughout, which is what localised this to the viewport wiring rather than to drawing code — and a pixel scan of the canvas region, not a glance at a screenshot, is what established that *nothing* was drawn rather than something small or low-contrast.
- [x] **Typing one letter no longer opens a list of everything containing that letter** — `FuzzyMatcher.Score` accepted any subsequence, which is fine at three characters and useless at one, and one is where the list is longest. Typing `x` inside an argument list matched `AccessViolationException`, `BoundingBox`, `DoubleExtensions` and hundreds more; the list being alphabetical (note 115), the row Tab would take was `AccessViolationException`. The **first** typed character must now begin a word — index 0, after `_` or `.`, or a capital opening a camelCase hump — while later characters stay a free subsequence, so `clr`→`Color`, `VPt`→`VPoint`, `p`→`VPoint` are untouched. Both hosts filter through `Score` (notes 14, 43), so one change covers them. `Editor/FuzzyMatcher.cs`, `Tests/RoslynCompletionTests.cs`. CLAUDE.md note 124.
- [x] **The shared editor host could show an empty completion popup** — it decided whether to open the window from the *unfiltered* Roslyn count and filtered afterwards, so when nothing survived it put an empty list on screen, covering the code and swallowing Enter and Tab. It now tests the filtered count, as `MainWindow` always has. `Editor/SharedEditorController.cs`.
- [x] **`VText.Justify` — how the lines of a multi-line label line up with each other** (`VTextJustify.Left`/`Center`/`Right`, default `Left`). It composes with `Anchor` rather than competing with it: `Anchor` puts the block on the drawing, `Justify` shapes the ragged edge inside it, so a label can be centred on its point *and* have its short lines centred against its long ones. Single-line text is unaffected, and justification never moves or resizes the block — `GetBounds()` is unchanged by it, which is pinned by a test. In the renderer, `TextAlignment` alone is inert: WPF aligns lines within `MaxTextWidth`, so that is set to the measured natural width first. Only the canvas lays multi-line text out as lines; PDF/SVG/DXF write the content as a single run, so justification does not reach them — a pre-existing limitation of those formats, stated in the README and F1 help rather than left to be discovered. `C2VGeometry/Enums/VTextJustify.cs`, `C2VGeometry/Shapes/VText.cs`, `Canvas/RenderCanvas.cs`, `Tests/TextJustifyTests.cs`. CLAUDE.md note 125.
...
- [x] **Auto-Run — a checkbox beside Run that re-runs the project every 500 ms** — off by default and saved per project in the `.vizproj` (`AutoRun`), so an armed project comes back armed and no other project is affected. Ticks are dropped rather than queued while a run is in flight; errors go to the status bar and the editor squiggles, never a dialog. Deliberately **not** note 120's auto-update returning: that ran on every keystroke, was on by default and was application-wide. CLAUDE.md note 121.
- [x] **A tick recompiles only when the source changed** — and that is correctness, not speed. `CompileAndExecuteAsync` clears the canvas *before* Roslyn runs, so recompiling every tick left the drawing blank for most of each cycle. An unchanged tick re-invokes the resident assembly (~16 ms), the same mechanism that makes a Global Parameters drag smooth; the shared helper is used by both rather than copied. The one visible consequence is documented: user `static` state resets when a tick recompiles — i.e. after an edit — not on every tick.
- [x] **Switching projects through the New/Open dialog never reloaded the Settings tab** — pre-existing, and found only because Auto-Run made it consequential. One `LoadSettingsToUI()` call at the path that lacked it.
- Worth keeping: the feature could only be confirmed by running it. A probe project with a `DateTime.Now` clock, captured twice four seconds apart, is what proved the canvas rebuilds with nobody pressing Run — and is what exposed the blank-canvas problem that all 1022 tests were happy with.
...
### Recently Completed (2026-08-22) — Seven Editor Faults from Live Use
- [x] **Multi-cursor Tab and paste** — Tab was never claimed by the multi-cursor handler, so AvalonEdit answered for the main caret alone and one keystroke indented the first line while outdenting the rest. `IndentAtAllCursors`/`OutdentAtAllCursors` now own it in both hosts; paste spreads one clipboard line per cursor when the counts match, instead of pasting the joined text everywhere. CLAUDE.md note 140, `Tests/MultiCursorEditingTests.cs`.
- [x] **Enter inside a single-line string literal** — a raw newline between the quotes does not compile, so the key that inserted one broke the file. `Editor/StringLiteralSplitter.cs` closes the literal and continues it on the next line; verbatim, raw, interpolation holes and mid-escape positions keep the plain Enter. Pure function, 15 tests, no window needed. CLAUDE.md note 139.
- [x] **Duplicate line left the caret past the copy** — an insert at the caret's own offset carries the caret with it, so the line adjustment double-counted. Both hosts capture the position first and assign it back; `CopyLineUp` had the mirror-image bug. CLAUDE.md note 138.
- [x] **Three completion contexts that were answering with the global list** — an argument list now offers values rather than every type, method and keyword in scope (with the restriction lifting after `new` and inside nested bodies); a property's accessor list offers `get;`/`set;` rather than `GetHashCode`; and a property initialiser or parameter default now knows its declared type, so `= new ` opens on `List<string>`. CLAUDE.md notes 141, 142, `Tests/CompletionContextTests.cs`.
- [ ] **Not verified by hand yet** — every one of these is a keystroke in a live `TextArea`, which the suite cannot drive. The pure decisions are unit-tested and both hosts are pinned by source scans, but the keystrokes themselves want a few minutes in the running app.
...
Found by the 2026-08-22 pre-release documentation pass (two agents, reflection dump). Each needs a
decision rather than a code change, which is why none is fixed.

- [ ] **A name-keyed member description cannot distinguish overloads, and twelve curve types show the wrong text for one of them.** Every curve has both `ICurve Offset(double)` and `List<ICurve> Offset(List<double>)`; F1 Help renders two rows and gives both the single-distance description, so the row whose signature is `List<ICurve> Offset(List<Double> distances)` reads "Creates a parallel line offset by the specified distance." `_memberDescriptions["ICurve.Offset"]` does describe the list overload properly, so the fix is either to key descriptions by signature the way constructors already are (note 91) or to make each per-type entry cover both. Uniform across all twelve types, so it reads as an editorial choice rather than drift.
- [ ] **`Shape.ToString` has a description but `Shape` declares no `ToString`.** It resolves to `System.Object.ToString`, so the reachability tests pass and the page renders, but the description promises formatting the base implementation does not produce. `VLine.ToString` is fine — `VLine` really does declare one. The question is whether the `Shape` entry should exist at all.
- [ ] **`C2VGeometry.Rendering` has no README section.** `IPrimitiveSink`, `ShapeTessellator`, `PenSpec`, `TessellationHints`, `FillRule`, `BoundsPrimitiveSink` and `PolylineFallbackSink` are public, in scope for F1 Help, and documented there — but the README never names them. They are the tessellation seam an exporter or backend author works against (note 81), so the decision is whether renderer-author types get a README section or are explicitly declared outside the user narrative. The other unnamed types are either `DoodleSharp.Canvas` internals (out of README scope by design) or extension holders whose methods are documented under their call syntax.
