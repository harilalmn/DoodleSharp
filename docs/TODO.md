...
- [ ] **Open follow-up: existing projects are not auto-migrated.** The rename happens at project-creation time only, so a project created before this release keeps its shadowing namespace until it is edited by hand (now safe, thanks to the entry-point scan). Auto-rewriting user source on project open was judged too surprising to do silently; if it is ever wanted it needs a prompt and an undo story.
